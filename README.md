...
[x] 排行榜可以首页进入
- 实验五：实现排行榜功能
- 实验六：继续开发网络功能（已完成基于 Socket 的实时比分同步）

## 实验六联机说明

本仓库的联机功能采用最小 Socket 方案：

- 每位玩家仍运行本地单机循环
- 联机只同步 `score`、`alive/dead` 和最终结果
- 不同步敌机、子弹、坐标

这样可以满足实验六要求，同时保持现有 `BaseGame` 主循环稳定。
...
```bash
javac server/OnlineBattleServer.java
java -cp server OnlineBattleServer
```
...
- 两位玩家连上后，HUD 可看到对手分数和连接状态
- 一方得分变化时，另一方 HUD 会实时刷新
- 双方都死亡后，双方都会收到统一结算弹窗
